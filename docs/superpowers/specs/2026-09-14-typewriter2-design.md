...
| 22 | Timing for a snippet whose language has no comment syntax | A per-directory sidecar, `.typewriter.json`, mapping relative path to the directive fields. The comment header stays the ONLY source of truth wherever comments exist, so the two never compete; the sidecar is consulted only when `CommentSyntax.hasAny` is false. `write` must not synthesise an uncommented marker line in that case — `MarkerScanner` cannot recognise it, so it would be typed into the editor as ordinary text. `pause` and `action` are unavailable for such snippets by construction, since a command is a marker and a marker is a comment. The sidecar is excluded from snippet collection by exact name. Rejected: session-only values, which lose a speaker's setting on every restart; a settings-level map, which does not travel with the demo repository; and disabling the fields, which removes per-snippet control from exactly the plain-text and YAML snippets the rewrite exists to serve. |
