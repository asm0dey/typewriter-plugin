...
| 23 | Naming a snippet whose file type has no usable extension | Prefer an extension whenever the type has one: `stem` plus that extension, at the snippet-directory root. Only when a type is **exact-only** — it has `ExactFileNameMatcher`s and no `ExtensionFileNameMatcher` — does the stem name a **directory** instead, with the exact name inside it: stem `01-setup` plus EditorConfig gives `01-setup/.editorconfig`. The exact-only set is **discovered at runtime from `getAssociations()`, never hardcoded**, because it differs per IDE and per installed plugin: IntelliJ IDEA Ultimate 2025.2 has 16 such non-binary types (`yarn.lock`, `tsconfig`, `.editorconfig`, `module-info.java`, `maven.config`, `lombok.config` among them), while RubyMine adds `Gemfile`, `Rakefile` and `Podfile`, and PyCharm adds `Pipfile` and `Procfile`. The plugin depends only on `com.intellij.modules.platform` and so runs in all of them; a hardcoded list would be wrong in each one differently. Those counts are observations, not a contract. Dockerfile is **not** one of them — it carries nine exact matchers plus `*.dockerfile`, so `01-build.dockerfile` resolves correctly and needs no directory. Beware `defaultExtension`: it is empty for Dockerfile despite that extension matcher existing, so the extension must be read from the associations, not from `defaultExtension`. Ignore-files need nothing special — `.gitignore` and `.dockerignore` are `EXTENSION *.gitignore`/`*.dockerignore`, so `01-setup.gitignore` already works. Without the directory rule an exact-only type could appear once per talk and take no `01-` sequence position. Rejected: stripping a `\d+-` prefix before resolving the file type, which would make the parser see one language while the editor tab sees plain text — the divergence section 5 forbids; and registering our own association, which writes into the user's global settings as v1 did with the keymap. |
